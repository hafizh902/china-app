...
<nav class="bg-white shadow-lg sticky top-0 z-50 border-b border-gray-100">
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="flex items-center space-x-8">
                ...
                <div class="hidden lg:flex space-x-1">
                    <a href="{{ route('home') }}" class="px-4 py-2 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium transition-all duration-200 rounded-lg {{ request()->routeIs('home') ? 'text-red-600 bg-red-50' : '' }}">
                        <i class="fas fa-home mr-2"></i>Home
                    </a>
                    <a href="{{ route('menu') }}" class="px-4 py-2 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium transition-all duration-200 rounded-lg {{ request()->routeIs('menu') ? 'text-red-600 bg-red-50' : '' }}">
                        <i class="fas fa-utensils mr-2"></i>Menu
                    </a>
                    <a href="{{ route('orders') }}" class="px-4 py-2 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium transition-all duration-200 rounded-lg {{ request()->routeIs('orders') ? 'text-red-600 bg-red-50' : '' }}">
                        <i class="fas fa-shopping-bag mr-2"></i>Orders
                    </a>
                    @auth
                        @if(auth()->user()->role === 'admin')
                            <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium transition-all duration-200 rounded-lg {{ request()->routeIs('admin.*') ? 'text-red-600 bg-red-50' : '' }}">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        >
                            ...
                            <div class="hidden sm:block text-left">
                                ...
                                <div class="text-xs text-gray-500">{{ auth()->user()->role === 'admin' ? 'Administrator' : 'Customer' }}</div>
                            </div>
                            <!-- Dropdown Icon -->
                        ...
                        >
                            ...
                            <!-- Menu Items -->
                            <div class="py-2">
                                <a href="{{ route('user.settings') }}" class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors">
                                    <i class="fas fa-user-cog mr-3 text-gray-400"></i>
                                    <span>Pengaturan</span>
                                </a>

                                @if(auth()->user()->role === 'admin')
                                    <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors">
    ...
    <div class="lg:hidden border-t border-gray-100 bg-gray-50 px-4 py-3">
        <div class="flex justify-around">
            <a href="{{ route('home') }}" class="flex flex-col items-center text-gray-600 hover:text-red-600 transition-colors {{ request()->routeIs('home') ? 'text-red-600' : '' }}">
                <i class="fas fa-home text-lg mb-1"></i>
                <span class="text-xs">Home</span>
            </a>
            <a href="{{ route('menu') }}" class="flex flex-col items-center text-gray-600 hover:text-red-600 transition-colors {{ request()->routeIs('menu') ? 'text-red-600' : '' }}">
                <i class="fas fa-utensils text-lg mb-1"></i>
                <span class="text-xs">Menu</span>
            </a>
            <a href="{{ route('orders') }}" class="flex flex-col items-center text-gray-600 hover:text-red-600 transition-colors {{ request()->routeIs('orders') ? 'text-red-600' : '' }}">
                <i class="fas fa-shopping-bag text-lg mb-1"></i>
                <span class="text-xs">Orders</span>
            </a>
            @auth
                <a href="{{ route('user.settings') }}" class="flex flex-col items-center text-gray-600 hover:text-red-600 transition-colors">
...
<div id="logout-confirm-modal"
     ...
     style="display: {{ $showLogoutConfirm ? 'flex' : 'none' }}; background-color: rgba(0, 0, 0, {{ $showLogoutConfirm ? '0.5' : '0' }});"
