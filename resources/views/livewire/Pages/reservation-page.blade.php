<div class="w-full bg-[#fdfcf8] min-h-screen pb-20 font-sans">
    ...
    <div class="max-w-6xl mx-auto px-4 pt-10 mb-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-stone-200 pb-6">
            <div>
                <h1 class="font-serif font-black text-4xl text-stone-800 tracking-tight">{{ __('language.reservation') }}
                </h1>
                <p class="text-stone-500 uppercase tracking-[0.3em] text-[10px] font-bold mt-2">
                    {{ __('language.dragon_kitchen_experience') }}</p>
            </div>
            ...
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2">
                    <span
                        class="w-8 h-8 rounded-full {{ $step == 1 ? 'bg-red-700 text-white' : 'bg-green-100 text-green-700' }} flex items-center justify-center text-xs font-bold shadow-sm">
                        @if ($step > 1)
                    ...
                    <span
                        class="text-[10px] font-black uppercase tracking-widest {{ $step == 1 ? 'text-red-700' : 'text-stone-400' }}">{{ __('language.select_table') }}</span>
                </div>
                <div class="w-8 h-[2px] bg-stone-200"></div>
                <div class="flex items-center gap-2">
                    <span
                        class="w-8 h-8 rounded-full {{ $step == 2 ? 'bg-red-700 text-white' : 'bg-stone-100 text-stone-400' }} flex items-center justify-center text-xs font-bold shadow-sm">2</span>
                    <span
                        class="text-[10px] font-black uppercase tracking-widest {{ $step == 2 ? 'text-red-700' : 'text-stone-400' }}">{{ __('language.payment') }}</span>
                </div>
            </div>
        </div>
    </div>
                ...
                <div
                    ...
                    <div class="flex items-center justify-between mb-10">
                        <h2 class="font-serif font-bold text-2xl text-stone-800 italic">
                            {{ __('language.restaurant_layout') }}</h2>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
                        @foreach ($tables as $table)
                            <div
                                class="group relative bg-stone-50 rounded-[2rem] p-6 border-2 transition-all duration-300 {{ $selectedTable == $table->id ? 'border-red-700 shadow-lg ring-4 ring-red-50' : 'border-transparent hover:border-amber-200 hover:bg-white' }}">

                                {{-- Table Header --}}
                                <div class="flex justify-between items-start mb-6">
                                    <div
                                        class="w-14 h-14 rounded-2xl {{ $selectedTable == $table->id ? 'bg-red-700 text-white' : 'bg-white text-stone-800 shadow-sm' }} flex items-center justify-center transition-colors">
                                        <div class="text-center">
                                            <span
                                                class="block text-[10px] uppercase font-black opacity-60">{{ __('language.table') }}</span>
                    ...
                    <div class="sticky top-10 space-y-6">
                        <div class="bg-stone-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
                            ...
                            <h3
                                ...
                                {{ __('language.reservation_info') }}</h3>

                            <div class="relative z-10 space-y-6">
                                <div class="space-y-2">
                                    <label
                                        class="text-[10px] text-stone-400 font-bold uppercase tracking-widest">{{ __('language.select_date') }}</label>
                                ...
                                @if ($selectedTable && $selectedTime)
                                    <div class="p-6 bg-stone-800/50 border border-white/10 rounded-2xl animate-fade-in">
                                        <div
                                            ...
                                            <span
                                                class="text-xs text-stone-400">{{ __('language.selected_table') }}</span>
                                            <span class="text-sm font-black text-amber-400">No.
                                                {{ $tables->where('id', $selectedTable)->first()->number ?? '' }}</span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span
                                                class="text-xs text-stone-400">{{ __('language.arrival_time') }}</span>
                                ...
                                @else
                                    <div class="p-8 border-2 border-dashed border-stone-700 rounded-2xl text-center">
                                        <p class="text-[10px] text-stone-500 font-bold uppercase tracking-widest">
                                            {{ __('language.select_table_time') }}</p>
                                    </div>
                                @endif
                        ...
                        <div class="bg-amber-50 p-6 rounded-2xl border border-amber-100">
                            <h4 class="text-xs font-black text-amber-900 uppercase mb-2">{{ __('language.need_help') }}
                            </h4>
                            <p class="text-xs text-amber-800/70 leading-relaxed font-medium">
                                {{ __('language.contact_whatsapp') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[3rem] shadow-2xl border border-stone-200 overflow-hidden">
                ...
                <div class="p-10">
                    <div
                        ...
                        <div>
                            <p class="text-[10px] text-stone-400 uppercase font-black tracking-[0.2em] mb-1">
                                {{ __('language.remaining_payment_time') }}</p>
                    ...
                    <div class="space-y-4 mb-10">
                        <label
                            class="text-[10px] text-stone-400 font-black uppercase tracking-widest ml-1">{{ __('language.transfer_instructions') }}</label>
                    ...
                    <div
                        ...
                        <span
                            class="text-xs font-black text-stone-600 uppercase tracking-widest">{{ __('language.total_deposit') }}</span>
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        <button
                            ...
                            <a href="https://wa.me/{{ \App\Models\SystemConfig::get('footer_phone')[0]['footer_phone'] ?? '' }}"
