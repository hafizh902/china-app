<div class="w-full bg-[#fdfcf8] min-h-screen pb-20 font-sans overflow-x-hidden">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-stone-200 pb-8">
            ...
            <div class="flex items-center gap-2 md:gap-4 bg-white p-3 rounded-2xl shadow-sm border border-stone-100">
                <div class="flex items-center gap-2">
                    <span class="w-8 h-8 rounded-full {{ $step == 1 ? 'bg-red-700 text-white shadow-lg shadow-red-200' : 'bg-green-500 text-white' }} flex items-center justify-center text-xs font-bold transition-all">
                        @if ($step > 1) <i class="fas fa-check text-[10px]"></i> @else 1 @endif
                    </span>
                    <span class="text-[10px] font-black uppercase tracking-widest {{ $step == 1 ? 'text-red-700' : 'text-stone-400' }}">
                        {{ __('language.select_table') }}
                    </span>
                </div>
                <div class="w-6 md:w-10 h-[1px] bg-stone-200"></div>
                <div class="flex items-center gap-2">
                    <span class="w-8 h-8 rounded-full {{ $step == 2 ? 'bg-red-700 text-white shadow-lg shadow-red-200' : 'bg-stone-100 text-stone-400' }} flex items-center justify-center text-xs font-bold transition-all">
                        2
                    </span>
                    <span class="text-[10px] font-black uppercase tracking-widest {{ $step == 2 ? 'text-red-700' : 'text-stone-400' }}">
                        {{ __('language.payment') }}
                    </span>
                </div>
            </div>
    ...
    @if ($step == 1)
        <div class="max-w-6xl mx-auto px-4">
            <div class="flex flex-col lg:grid lg:grid-cols-12 gap-8 md:gap-12">
                ...
                {{-- A. SIDEBAR (Di Mobile tampil PERTAMA agar pilih tanggal dulu) --}}
                    ...
                    <div class="lg:sticky lg:top-10 space-y-6">
                        <div class="bg-stone-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
                            ...
                            <h3 class="relative z-10 text-[10px] font-black uppercase tracking-[0.3em] text-amber-400 mb-8 flex items-center gap-2">
                                ...
                                {{ __('language.reservation_info') }}
                            </h3>

                            <div class="relative z-10 space-y-6">
                                <div class="space-y-3">
                                    <label class="text-[10px] text-stone-400 font-bold uppercase tracking-widest ml-1">{{ __('language.select_date') }}</label>
                                ...
                                @if ($selectedTable && $selectedTime)
                                    <div class="p-6 bg-stone-800/80 border border-white/10 rounded-2xl animate-fade-in ring-1 ring-amber-500/30">
                                        <div class="flex justify-between items-center mb-4 pb-4 border-b border-white/5">
                                            <span class="text-xs text-stone-400 uppercase tracking-tighter">{{ __('language.selected_table') }}</span>
                                            <span class="text-sm font-black text-amber-400">MEJA #{{ $tables->where('id', $selectedTable)->first()->number ?? '' }}</span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-xs text-stone-400 uppercase tracking-tighter">{{ __('language.arrival_time') }}</span>
                                ...
                                @else
                                    <div class="p-6 border-2 border-dashed border-stone-700 rounded-2xl text-center">
                                        <p class="text-[10px] text-stone-500 font-bold uppercase tracking-widest leading-loose">
                                            {{ __('language.select_table_time') }}
                                        </p>
                                    </div>
                                @endif

                                <button wire:click="confirmReservation" @disabled(!$selectedTable || !$selectedTime)
                                    class="w-full py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-[11px] transition-all
                                    {{ !$selectedTable || !$selectedTime ? 'bg-stone-800 text-stone-600 cursor-not-allowed' : 'bg-red-700 hover:bg-red-600 text-white shadow-xl shadow-red-900/40 active:scale-95' }}">
                                    {{ __('language.confirm_booking') }}
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="bg-amber-50/50 p-6 rounded-[2rem] border border-amber-100 flex items-center justify-between group">
                            ...
                            <a href="https://wa.me/{{ \App\Models\SystemConfig::get('footer_phone')[0]['footer_phone'] ?? '' }}" target="_blank" class="w-8 h-8 rounded-full bg-stone-900 text-white flex items-center justify-center hover:scale-110 transition-transform">
                ...
                {{-- B. DENAH RESTORAN (Order 2 di Mobile) --}}
                <div class="lg:col-span-8 order-2 lg:order-1 bg-white rounded-[2.5rem] md:rounded-[3.5rem] shadow-sm border border-stone-200 p-6 md:p-12 relative overflow-hidden">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
                        @foreach ($tables as $table)
                            <div class="group relative bg-stone-50 rounded-[2rem] p-6 border-2 transition-all duration-500 {{ $selectedTable == $table->id ? 'border-red-700 shadow-xl ring-4 ring-red-50 bg-white' : 'border-transparent hover:border-amber-200 hover:bg-white' }}">
                                
                                <div class="flex justify-between items-start mb-6">
                                    <div class="w-14 h-14 rounded-2xl {{ $selectedTable == $table->id ? 'bg-red-700 text-white' : 'bg-white text-stone-800 shadow-sm' }} flex items-center justify-center transition-all duration-500">
                                        <div class="text-center">
                                            <span class="block text-[10px] uppercase font-black opacity-60">{{ __('language.table') }}</span>
                                    ...
                                    <div class="flex flex-col items-end gap-1">
                                        <span class="px-3 py-1 bg-amber-100 text-amber-800 rounded-full text-[10px] font-black uppercase tracking-tighter shadow-sm">
                                            {{ $table->capacity }} {{ __('language.pax') }}
                                        </span>
                                ...
                                <div class="grid grid-cols-3 md:grid-cols-2 gap-2">
                                    ...
                                    @foreach (['10:00', '12:00', '14:00', '16:00', '18:00', '20:00'] as $time)
                                        ...
                                        @endphp
                                        <button
                                            wire:click="selectTableAndTime({{ $table->id }}, '{{ $time }}')"
    ...
    @else
        {{-- 2. BAGIAN PEMBAYARAN (Full Completeness) --}}
            ...
            <div class="bg-white rounded-[3rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-stone-200 overflow-hidden">
                ...
                <div class="bg-red-800 p-10 text-center relative">
                    ...
                    <h2 class="text-white font-serif font-bold text-3xl tracking-widest uppercase italic relative z-10">
                        {{ __('language.deposit_validation') }}
                    </h2>
                ...
                <div class="p-8 md:p-12">
                    ...
                    <div class="flex items-center justify-between mb-10 bg-stone-50 p-6 rounded-[2rem] border border-stone-100 shadow-inner">
                        <div>
                            <p class="text-[10px] text-stone-400 uppercase font-black tracking-[0.2em] mb-1">
                                {{ __('language.remaining_payment_time') }}
                            </p>
                    ...
                    <div class="space-y-4 mb-10">
                        <label class="text-[10px] text-stone-400 font-black uppercase tracking-widest ml-1">
                            {{ __('language.transfer_instructions') }}
                        </label>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <a href="https://wa.me/{{ \App\Models\SystemConfig::get('footer_phone')[0]['footer_phone'] ?? '' }}?text=Halo%20Admin,%20Saya%20ingin%20mengirimkan%20bukti%20pembayaran%20reservasi."
                           target="_blank"
