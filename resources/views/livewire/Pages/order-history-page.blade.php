                ...
                <div class="flex flex-wrap justify-between items-center bg-stone-50 px-8 py-5 border-b border-amber-50">
                    <div class="flex gap-8">
                        <div>
                            ...
                            <p class="font-serif font-bold text-stone-800 italic">{{ $order->created_at->format('d M, Y') }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] text-stone-400 uppercase font-bold tracking-widest mb-1">Total</p>
                            <p class="font-bold text-red-700">Rp{{ number_format($order->total, 0, ',', '.') }}</p>
                        </div>
                        ...
                        @foreach ($order->items as $item)
                            <div class="flex items-center gap-4 p-3 rounded-2xl hover:bg-amber-50/50 transition-colors border border-transparent hover:border-amber-100">
                                <div class="w-16 h-16 bg-stone-100 rounded-xl flex-shrink-0 border border-stone-200 overflow-hidden">
                                    <img src="{{ $item->menu_image_url ?? asset('images/default-dish.jpg') }}" 
                                         class="w-full h-full object-cover opacity-80" alt="{{ $item->menu_name }}">
                                </div>
                                <div class="flex-1">
                                    <p class="font-serif font-bold text-stone-800 leading-tight">{{ $item->menu_name }}</p>
                                    <p class="text-xs text-stone-400 mt-1 uppercase tracking-widest">Jumlah: {{ $item->quantity }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-bold text-stone-700">Rp{{ number_format($item->subtotal, 0, ',', '.') }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="p-8 bg-white border-t-8 border-red-800 relative" id="invoice">
                ...
                <div class="grid grid-cols-2 gap-8 py-8 text-sm relative z-10">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-stone-500 text-xs italic">{{ $selectedOrder->created_at->format('H:i T') }}</p>
                    </div>
                </div>
                            ...
                            @foreach ($selectedOrder->items as $item)
                                <tr class="border-b border-stone-100">
                                    <td class="py-4 px-2">
                                        ...
                                        <p class="text-[10px] text-stone-400">@ Rp{{ number_format($item->price, 0, ',', '.') }}</p>
                                    </td>
