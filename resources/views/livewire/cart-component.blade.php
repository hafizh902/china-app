                        ...
                        <div class="flex-1 flex flex-col justify-between py-0.5">
                            ...
                            <div class="flex justify-between items-end mt-3">
                                ...
                                <div
                                    ...
                                    <span
                                        class="text-xs font-black text-stone-800 w-8 text-center">{{ $item['quantity'] }}</span>
                                    <button
                                        wire:click="updateQuantity('{{ $id }}', {{ $item['quantity'] + 1 }})"
                                        class="w-7 h-7 flex items-center justify-center rounded-full bg-white text-stone-400 hover:text-red-700 hover:shadow-sm transition-all text-[10px]">
                ...
                <div
                    ...
                    <div class="space-y-3 mb-8">
                        <div class="flex justify-between text-xs text-stone-400 font-medium tracking-wide">
                            <span> {{ __('language.subtotal_payment') }}</span>
                            <span>Rp{{ number_format($subtotal, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-xs text-stone-400 font-medium tracking-wide">
                            <span> {{ __('language.tax') }}
                                ({{ \App\Models\SystemConfig::value('tax_percent') ?? '-' }}%)</span>
                        ...
                        <div class="flex justify-between items-end">
                            <div>
                                <span
                                    class="text-[10px] text-red-700 uppercase font-black tracking-[0.2em] block mb-1">{{ __('language.total_pay') }}</span>
                            ...
                            <div
                                ...
                                <i class="fas fa-sparkles mr-1"></i>{{ __('language.not_included_delivery') }}
                            </div>
                        </div>
                    </div>
                    @auth
                        <button wire:click="closeCart" wire:navigate href="{{ route('checkout') }}"
                            ...
                            <span
                                class="relative z-10 font-black uppercase tracking-[0.3em] text-[11px]">{{ __('language.checkout') }}</span>
                            <i
