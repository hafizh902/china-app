        ...
        <div class="col-span-12 lg:col-span-9">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div wire:key="menu-{{ $item->id }}"
                    ...
                    {{-- Overlay Delete Mode --}}
                    @if ($deleteMode)
                    <div class="absolute inset-0 bg-red-900/10 z-30 cursor-pointer flex items-start p-4" wire:click="$toggle('selectedItems.{{ $item->id }}')">
                    ...
                    <div class="relative h-48 overflow-hidden bg-stone-100">
                        <span class="absolute top-4 right-4 z-20 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm {{ $item->is_available ? 'bg-green-100 text-green-700' : 'bg-stone-200 text-stone-500' }}">
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-stone-900/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                            @if (!$deleteMode)
                            <button wire:click="edit({{ $item->id }})" class="w-full py-2 bg-white/95 backdrop-blur text-stone-900 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-red-700 hover:text-white transition-colors">
                                Edit Menu
                            </button>
                            @endif
                        </div>
                    ...
                    <div class="p-5 flex-1 flex flex-col">
                        <div class="flex justify-between items-start mb-2">
                            <span class="text-[10px] font-black text-red-700 uppercase tracking-[0.2em]">{{ str_replace('_', ' ', $item->category_label) }}</span>
                        </div>
                        <h3 class="font-serif font-bold text-stone-800 text-lg leading-tight mb-1 truncate">{{ $item->name }}</h3>
                        <p class="text-xs text-stone-500 line-clamp-2 mb-4 leading-relaxed italic">{{ $item->description }}</p>
                        ...
                        <div class="mt-auto pt-4 border-t border-stone-100 flex justify-between items-center">
                            <span class="text-lg font-black text-stone-900 tracking-tighter">
                                <span class="text-xs font-medium text-stone-400 mr-1">Rp</span>{{ number_format($item->price, 0, ',', '.') }}
                            </span>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-span-12 lg:col-span-3 space-y-6">
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-stone-200 p-6 sticky top-8">
                ...
                <div class="mt-8 flex justify-between items-center bg-stone-50 p-2 rounded-2xl">
                    <button wire:click="previousPage" @disabled($items->onFirstPage()) class="w-10 h-10 flex items-center justify-center rounded-xl {{ $items->onFirstPage() ? 'text-stone-300' : 'text-stone-600 hover:bg-white' }}">
                        <i class="fas fa-chevron-left text-xs"></i>
                    </button>
                    <span class="text-xs font-black text-stone-800 uppercase tracking-widest">Hal {{ $items->currentPage() }}</span>
                    <button wire:click="nextPage" @disabled(!$items->hasMorePages()) class="w-10 h-10 flex items-center justify-center rounded-xl {{ !$items->hasMorePages() ? 'text-stone-300' : 'text-stone-600 hover:bg-white' }}">
