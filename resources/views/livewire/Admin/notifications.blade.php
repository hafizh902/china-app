    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div class="bg-white border-b-4 border-chinese-red p-5 shadow-sm">
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <span class="text-3xl font-black text-chinese-black">{{ $orders->where('created_at', '>=', today())->count() }}</span>
            ...
            <div class="bg-white border-b-4 border-chinese-gold p-5 shadow-sm">
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <span class="text-3xl font-black text-chinese-black">{{ $reservations->where('created_at', '>=', today())->count() }}</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <div class="space-y-3">
                    @forelse($orders->where('created_at', '>=', today()) as $order)
                        @php $isNew = $order->status === 'preparing'; @endphp
                        <div class="bg-white group border border-stone-200 p-4 transition-all hover:border-chinese-red relative overflow-hidden {{ !$isNew ? 'opacity-50' : '' }}">
                            <div class="flex justify-between items-center relative z-10">
                                ...
                                <a href="{{ route('admin.orders') }}" class="text-[9px] font-black bg-stone-100 px-3 py-2 group-hover:bg-chinese-red group-hover:text-white transition-colors uppercase">
                                    Detail
                                </a>
                            </div>
                        </div>
            ...
            <div>
                ...
                <div class="space-y-3">
                    @forelse($reservations->where('created_at', '>=', today()) as $res)
                        @php $isNew = $res->status === 'pending'; @endphp
                        <div class="bg-white border border-stone-200 p-4 transition-all hover:border-chinese-gold flex justify-between items-center {{ !$isNew ? 'opacity-50' : '' }}">
                            <div class="flex items-center gap-4">
                                <div class="w-8 h-8 rounded-full bg-stone-50 flex items-center justify-center border border-stone-100">
                                    <i class="fas fa-chair text-[10px] {{ $isNew ? 'text-chinese-gold' : 'text-stone-300' }}"></i>
                                </div>
                                <div>
                                    <p class="text-[11px] font-black text-chinese-black uppercase tracking-tight">Meja {{ $res->table_id }} <span class="text-stone-400 font-medium">— {{ $res->user->name ?? 'Guest' }}</span></p>
                                    <p class="text-[9px] font-bold text-stone-500 uppercase">Sch: <span class="text-chinese-gold">{{ $res->reservation_time }}</span></p>
                                </div>
                            </div>
                            <div class="flex items-center gap-4">
                                <span class="text-[8px] font-black px-2 py-1 {{ $isNew ? 'text-chinese-gold border border-chinese-gold' : 'text-stone-400 border border-stone-200' }} uppercase">
                                    {{ $res->status }}
                                </span>
                                <a href="{{ route('admin.reservations') }}" class="text-stone-400 hover:text-chinese-black transition-colors">
