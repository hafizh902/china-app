    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($menuItems->count() > 0): ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $menuItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="group relative bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border border-amber-100">
                    
                    
                    <div class="relative h-64 overflow-hidden <?php echo e(!$item->is_available ? 'grayscale' : ''); ?>"
                         <?php if($item->is_available): ?> wire:click="$dispatch('preview-modal', <?php echo e(json_encode($item)); ?>)" <?php endif; ?>>
                        
                        <img src="<?php echo e($item->image_url); ?>" alt="<?php echo e($item->name); ?>"
                             class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 <?php echo e($item->is_available ? 'cursor-pointer' : 'cursor-not-allowed'); ?>">
                        ...
                        <div class="absolute top-4 left-4">
                            <span class="px-3 py-1 rounded-full text-[10px] uppercase tracking-widest font-bold text-white shadow-lg <?php echo e($item->is_available ? 'bg-red-600 border border-amber-400' : 'bg-gray-500'); ?>">
                    ...
                    <div class="p-5 relative">
                        ...
                        <div class="absolute top-0 right-0 w-16 h-16 opacity-5 pointer-events-none group-hover:opacity-10 transition-opacity">
                        ...
                        <div class="flex justify-between items-start">
                            <div>
                                <h3 class="font-serif text-xl font-bold text-slate-800 group-hover:text-red-700 transition-colors uppercase tracking-tight">
                                    <?php echo e($item->name); ?>

                                </h3>
                                <div class="w-8 h-1 bg-red-600 mt-2 transition-all duration-500 group-hover:w-full"></div>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 flex justify-end">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->is_available): ?>
                                <button
                                    ...
                                    class="relative overflow-hidden group/btn bg-red-700 hover:bg-red-800 text-white flex items-center gap-2 px-5 py-2.5 rounded-xl transition-all shadow-lg active:scale-95">
                                    <span class="text-sm font-bold uppercase tracking-tighter">Add to Cart</span>
                                    <i class="fas fa-plus text-xs bg-amber-400 text-red-900 p-1 rounded-full"></i>
                                </button>
                            <?php else: ?>
                                <span class="text-gray-400 text-xs font-bold uppercase italic">Out of Stock</span>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
                    </div>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$item->is_available): ?>
                        <div class="absolute inset-0 bg-white/40 backdrop-blur-[2px] flex items-center justify-center z-10">
                            <div class="bg-black/80 text-white px-4 py-2 rotate-12 border-2 border-amber-500 font-bold uppercase tracking-widest">
                                Sold Out
                            </div>
                        </div>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
