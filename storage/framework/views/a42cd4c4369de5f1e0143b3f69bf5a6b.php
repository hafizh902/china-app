<?php
      ...
      if (in_array($crop, $cropOptions)) {
          ...
          if (isset($gravity)) {
              ...
              if (Str::startsWith($gravity, 'compass:')) {
                  ...
                  $gravityImplementation = Resize::$cropFactor()
                      ->width($width ?? '')
                      ->height($height ?? '')
                      ->gravity(Gravity::compass(Compass::$gravityConstant()));
              } else {
                  /**
  ...
  if (isset($effect)) {
      ...
      if (str_contains($effect, 'cartoonify|')) {
          ...
          $retrieveFormattedImage->effect(
              Effect::cartoonify()->lineStrength($lineStrength)->colorReductionLevel($colorReductionLevel),
          );
      }
  ...
  if (isset($trim)) {
      ...
      if (gettype($trim) == 'string') {
          ...
          $retrieveFormattedImage->reshape(
              Reshape::trim()->colorSimilarity($colorSimilarity)->colorOverride($colorFormat),
          );
      }
  }
  ...
  if (isset($improveMode)) {
      ...
      if (gettype($improveMode) == 'string') {
          ...
          $retrieveFormattedImage->adjust(Adjust::improve()->mode(ImproveMode::$mode())->blend($blend));
      }
  }
  ...
  if (isset($shadow)) {
      ...
      if (gettype($shadow) == 'string') {
          ...
          $retrieveFormattedImage->effect(
              Effect::shadow()->strength($strength)->color($colorFormat)->offsetX($offsetX)->offsetY($offsetY),
          );
      }
  }
...
?>
<?php /**PATH D:\projek 12\china-app\vendor\cloudinary-labs\cloudinary-laravel\views\components\image.blade.php ENDPATH**/ ?>